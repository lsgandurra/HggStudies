	...
	for (int toy=0; toy<ntoys; toy++)
	{
		...
		for (map<string,RooAbsData*>::iterator it=toys.begin(); it!=toys.end(); it++)
		{
			...
			for (map< pair<string, RooAbsPdf*>, pair<string*, double*> >::iterator m=test_sbPdfs.begin(); m!=test_sbPdfs.end(); m++) 
			{
				...
				double bestMu = 0.0;
                                double bestMuNll = 1e30;
                                RooFitResult *nom = NULL;
                                for(double muIter = muLow; muIter < muHigh; muIter+=muStep)
                                {
                                        mu->setConstant(false);
                                        mu->setVal(muIter);
                                        mu->setConstant(true);
     
                                        nom = pdf->fitTo(*(it->second),PrintLevel(-1),PrintEvalErrors(-1),Warnings(false),Save(true));
                                        if(nom->minNll() < bestMuNll)
                                        {
                                                bestMuNll = nom->minNll();
                                                bestMu = muIter;
                                        }
