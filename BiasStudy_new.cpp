...
int main(int argc, char *argv[])
{
	...
	string filebkg = "CMS-HGG_massfacmva_legacy_defaultE_80_120_PTl036_PTs03_DynPtCut50.root"; 
	string filesig = "CMS-HGG_massfacmva_legacy_defaultE_80_120_PTl036_PTs03_DynPtCut50.root";
	string datafile = "config_massfacmva_legacy_defaultE_80_120_PTl036_PTs03_DynPtCut50_noChi2GF_v2.dat";
	int cat = 0;
	double expectSignal = 0.0;
	int expectSignalMass = 100;
	double muLow = -100.0;
	double muHigh = 100.0;
	double muStep = 0.1;
	bool blind = false;
	...
	for (int toy=0; toy<ntoys; toy++)
	{
		...
		for (map< pair<string, RooAbsPdf*>, pair<string*, double*> >::iterator it=sbPdfs.begin(); it!=sbPdfs.end(); it++)
		{
			...
			RooDataHist *toyHist = it->first.second->generateBinned(RooArgSet(*mass),dataBinned->sumEntries(),Name(Form("%s_%d",it->first.first.c_str(),toy)));
			...
			frame->Draw();
			//c1->Print(Form("%s/cat%d/mass_%d/toyHisto/%s_toyHisto%d_jobID%d.pdf",output.c_str(),cat,expectSignalMass,it.c_str(),toy,jobID));
			c1->Clear();	
			frame->Delete();
		...
		for (map<string,RooAbsData*>::iterator it=toys.begin(); it!=toys.end(); it++)
		{
			...
			for (map< pair<string, RooAbsPdf*>, pair<string*, double*> >::iterator m=test_sbPdfs.begin(); m!=test_sbPdfs.end(); m++) 
			{
				...
				int iterV = 0;	
				double minNLLBest = 1.e6;
				double muBest = 0.0;
				for(double v = -3.1; v <= 3.0; v+=muStep)
				{

					cout << endl << "// ----- iterV = " << iterV << " ----- //" <<endl;
					if(iterV == 0)
					{
						mu->setConstant(false);
						mu->setVal(0.0);
					...
					*/
					...
					if(iterV == 0) globalMinNLL = 2*nom->minNll();
					if(iterV == 0) bestFitVal = mu->getVal();
					if(iterV == 0) bestFitValError = mu->getError();
	
					cout << endl << "bestFitVal = " << bestFitVal;
					cout << endl << "bestFitValError = " << bestFitValError;
					cout << endl << "globalMinNLL = " << globalMinNLL;
					cout << endl << "mu = " << mu->getVal();
					cout << endl << "2*minNll = " << 2*nom->minNll();
					
					if(nom->minNll() < minNLLBest) 
					{
						minNLLBest = nom->minNll(); 
						//bias = (expectSignal - bestFitVal) / bestFitValError;
						bias = (expectSignal - v) / bestFitValError;
						muBest = v;
						if(iterV == 0) bias2 = (expectSignal - bestFitVal) / bestFitValError; 
					}	
					cout << endl << "muBest = " << muBest;
