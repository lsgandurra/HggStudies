...
int main(int argc, char *argv[])
{
	...
	for(int i = startCat; i < ncats; i++)
	{
		...
		vector < vector<int> > coverageVector;	
		...
		for(vector<string>::iterator truth = truthFunctions.begin(); truth != truthFunctions.end(); truth++)
			...
			vector<int> coverageVectorRow;
				...
				for(int j = massLow; j <= massHigh; j+=5)
				{
					...
					string chainIn = Form("%s/cat%d/mass_%d/Truth_sb_%s_Test_sb_%s.txt",output.c_str(),i,j,truth->c_str(),test->c_str());
					...
					while(! file.eof())
					{
						...
						//cout << endl << mu_temp;
						muTh1->Fill(mu_temp);
						coverageValues.push_back(mu_temp);	
					}
					file.close();
					...
					if(fabs(median) < 5.0) fit = (RooFitResult*) pdf_fit.fitTo(rooTh1,Save(true),Range(median-1.6,median+1.6)); //FIXME

					rooTh1.plotOn(frame);
					pdf_fit.plotOn(frame);
					frame->Draw();
	
					//c1->Print(Form("%s/cat%d/mass_%d/Bias_Truth_sb_%s_Test_sb_%s_roofit.png",output.c_str(),i,j,truth->c_str(),test->c_str()));    
                                        //c1->Clear();	
					
					/*
					TF1 * pdf = new TF1("pdf","gaus",muLow,muHigh);
					muTh1->Fit("pdf");
					muTh1->Draw("E1");
					pdf->Draw("SAMES");
					...
					*/
					...
					latexLabel.DrawLatex(0.17, 0.83,Form("median = %3.2f",median));
					//latexLabel.DrawLatex(0.17, 0.78,Form("#bias = %3.1f",bias));	

					c1->Print(Form("%s/cat%d/mass_%d/Bias_Truth_sb_%s_Test_sb_%s_gaus.pdf",output.c_str(),i,j,truth->c_str(),test->c_str()));
					c1->Print(Form("%s/cat%d/mass_%d/Bias_Truth_sb_%s_Test_sb_%s_gaus.png",output.c_str(),i,j,truth->c_str(),test->c_str()));	
					c1->Print(Form("%s/cat%d/mass_%d/Bias_Truth_sb_%s_Test_sb_%s_gaus.C",output.c_str(),i,j,truth->c_str(),test->c_str()));
					c1->Clear();
					...
					// --- coverage evaluation --- //
					int stat = 0;
					for(int cov = 0; cov < coverageValues.size(); cov++)
					{	
						if(coverageValues[i] > -sigma && coverageValues[i] < sigma) stat++;	

					}
					double coverage = ((double) stat) / ((double) coverageValues.size());
					if(coverage > 0.68) covSum++;
					

					coverageValues.erase(coverageValues.begin(),coverageValues.end());
					// --- end coverage evaluation --- //
				}

				// --- coverage --- //
				int nMasses = (massHigh - massLow) / 5 + 1;
				if(covSum == nMasses) coverageVectorRow.push_back(1);
				else coverageVectorRow.push_back(0); 
				// --- end coverage --- //
			...
			coverageVector.push_back(coverageVectorRow);
		...
		// --- coverage --- //
		fstream coverageFile;
		coverageFile.open(Form("%s/CombinedPlots/coverage_cat%d.txt",output.c_str(),i),ios::out);
		string coverageChain = "";
		for(int j = 0; j < nbTest; j++)
                {
                        for(int k = 0; k < nbTruth; k++)
                        {
                                if(coverageVector[k][j] == 0) coverageChain = "No";
                                else coverageChain = "OK";
                                if(k == 0) coverageFile << coverageChain;
                                else coverageFile << " & " << coverageChain;
                                if(nbTruth == 3 && k == 2) maxBiasFile << " & X ";
                        }
                        coverageFile << endl;
                }		
